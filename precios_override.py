...
Guarda un JSON con los valores actualizados de las 4 lineas PMD.
...
def get_lineas() -> list[dict]:
    """Devuelve las 4 lineas con sus precios actuales (override o default)."""
...
def save_lineas(lineas: list[dict]) -> bool:
    """Guarda las 4 lineas actualizadas. Devuelve True si OK."""
    try:
        OVERRIDE_PATH.parent.mkdir(parents=True, exist_ok=True)
        OVERRIDE_PATH.write_text(json.dumps(lineas, ensure_ascii=False, indent=2), encoding="utf-8")
        logger.info("Precios de lineas actualizados: %s", [l.get("base") for l in lineas])
        return True
    except Exception as e:
...
def get_precios_con_override() -> dict:
    """Devuelve el dict completo de precios con las lineas sobreescritas por el admin."""
